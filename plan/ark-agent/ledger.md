...
| 2026-08-20 | 12 | Command interface | DONE | app/cli.py, scripts/{ingest,query,digest,run}, tests/test_cli.py | ./scripts/test tests/test_cli.py | 5 passed | RED confirmed. End-to-end verified outside the suite: ./scripts/ingest over the fixture directory rejected both invalid packs with field-level errors, skipped the link-and-summarise source, and indexed 1 chunk. answer() and digest() then exercised against that real index: a real citation resolved, a fabricated one was dropped with two limitations recorded, a refusal was detected, and the digest cited correctly. |
| 2026-08-20 | 13 | Evaluation harness | DONE | evals/questions.yaml, evals/run_eval.py, evals/README.md | not part of ./scripts/test by design | n/a | Acceptance set ratified 2026-08-20: three per-organisation questions across four organisations, one cross-organisation question, one negative, plus the digest. Calls the real model, writes a reviewer sheet with citation URLs and empty accuracy fields. Never gates a unit. |
| 2026-08-20 | GATE | Phase 1 evaluation run | EVIDENCE | proposal/hackathon-1/execution/outputs/phase-1-review-sheet-2026-08-20.md | ./.venv/bin/python evals/run_eval.py --k 6 | 15 calls, 13 answered, 3 refusals, 0 uncited | Run on the server against the real model over the four-organisation corpus. Zero answers without a resolvable citation. Negative question refused with closest sources cited. Cross-organisation question answered with 3 citations; digest with 6 across organisations. Two Coherence refusals are honest and evidence-backed, not failures. Awaiting representative sign-off. |
