...
- **Document layer:** the existing `research/` directory in this git repo (markdown files) plus S3-compatible document storage
...
The system ingests from multiple document sources:

| Source | Method | Formats |
|---|---|---|
| `research/*.md` | Git-based via Hermes cron | Markdown |
| **S3-compatible storage** | `aws s3 sync` periodic pull | PDF, DOCX, HTML, TXT, MD |
| **Google Docs** | GDrive API export | Google Docs → DOCX |
| **Notion** | Notion API export | Notion pages → HTML/Markdown |
| **Obsidian** | File copy/symlink | Markdown, with wikilinks |
| **Telegram files** | Bot API webhook | PDF, DOCX, TXT, images (OCR) |

S3 documents are pulled periodically and stored at `~/.hermes/tcc/state/documents/s3/`.

### 4.3 Under the Hermes profile (runtime state, not git-tracked)

```
~/.hermes/<profile>/state/knowledge-base/
└── surreal-data/           # SurrealDB on-disk database files

~/.hermes/<profile>/state/documents/
├── s3/                     # S3-synced documents
└── sources.yml             # Source configuration (S3 buckets, GDrive folders)
```
...
### 4.4 Local to the skill but gitignored
...
### 5.3 Document Source Integrations

| Source | Integration Method | Notes |
|---|---|---|
| S3 | `aws s3 sync --exclude "*.md" documents/s3/` | Pull PDFs, DOCX, HTML |
| Google Docs | GDrive API + OAuth2 | Export to DOCX, then Kreuzberg extract |
...
### 5.5 Shell Scripts (Hermes Skill Interface)
...
### 5.7 Live Ingestion (Telegram file attachment)
...
**Supported formats:** PDF, DOCX, TXT, HTML, images (OCR), Markdown
...
### 5.8 Cron Trigger (replaces GitHub webhook)
...
Path B: S3 document sync
aws s3 sync → ~/.hermes/tcc/state/documents/s3/
        ↓
ingest.py per file:
  (Kreuzberg extract) → chunker → embedder → extractor → SurrealDB upsert
        ↓
Graph updated

Path C: Google Docs / Notion / Obsidian
GDrive API / Notion API / File watcher
        ↓
Export to temp file
        ↓
ingest.py:
  (Kreuzberg extract if needed) → chunker → embedder → extractor → SurrealDB upsert
        ↓
Graph updated

Path D: Live Telegram ingestion
Telegram file attachment
        ↓
Hermes Telegram gateway webhook
        ↓
